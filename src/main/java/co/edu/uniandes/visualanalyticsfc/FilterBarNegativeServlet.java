 ...
 * @author Deisy
 */
@WebServlet(name = "FilterBarNegativeServlet", urlPatterns = {"/FilterBarNegativeServlet"})
public class FilterBarNegativeServlet extends HttpServlet {
    ...
    private List<GroupFiltersDTO> getGeneralFilters(FilterDTO filter){
        ...
        for(Document var : varGroup){    
            ...
            for(int i=0; i< lstLabelsAge.length;i++){
                ...
                Bson group = new Document("$group", new Document("_id", "$"+var.get("variable").toString()+"."+lstLabelsAge[i])
                ...
                for (Document doc : docGroups) {
                    ...
                    groupDto.setVariable(lstLabelsAge[i]);
                    groupDto.setLabel(doc.get("_id").toString());
                    groupDto.setValue(doc.get("cont").toString());
                    if(doc.get("_id").toString().equals("1")){
                        int negative = Integer.valueOf(doc.get("cont").toString());
                        negative = negative*-1;
                        groupDto.setValue(String.valueOf(negative));
                    }
